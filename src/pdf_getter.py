...
def url_gen():
    ...
    root = 'http://www.equibase.com/premium/eqbPDFChartPlus.cfm?'
    params = 'RACE=A&BorP=P&TID={}&CTRY=USA&DT={}/{}/{}&DAY=D&STYLE=EQB'
    for track in TRACK_CODES:
        dates = date_range(track[1], track[2])
        for d in dates:
            yield root + params.format(track[0], d.month, d.day, d.year), track[0], d
